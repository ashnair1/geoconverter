...
def check_compatibility_status(gdal_version_str):
    ...
    try:
        ...
        for tested in tested_versions:
            ...
            if gdal_ver.major == tested_ver.major and gdal_ver.minor == tested_ver.minor:
                return "tested"
...
def check_breaking_changes(release_body):
    ...
    found_indicators = [indicator for indicator in breaking_indicators 
...
def generate_report():
    ...
    for release in releases:
        ...
        print(f"{status_icon} {breaking_icon} GDAL {version_str:8} | {status:6} | {release['published_at'][:10]}")
        
        if breaking_changes:
            print(f"    Breaking change indicators: {', '.join(breaking_changes)}")
            needs_attention.append((version_str, f"potential breaking changes: {', '.join(breaking_changes)}"))
